...
{
	public static final String QUOTEMARK = Character.toString('"');
					...
					for(Entry<String, Property> e : cat.entrySet())
						if(e!=null)
						{
							...
							if(data == null || data.length<7)
								break;
							String modelPath = data[0].replaceAll(QUOTEMARK, "");
							String partName = data[1].replaceAll(QUOTEMARK, "");
							String texture = data[2].replaceAll(QUOTEMARK, "");
							String[] sTransl = data[3].replaceAll(QUOTEMARK, "").split("_");
							if(sTransl.length<3)
								break;
							...
							String[] sRot = data[4].replaceAll(QUOTEMARK, "").split("_");
							if(sRot.length<3)
								break;
							...
							String[] sColour = data[5].replaceAll(QUOTEMARK, "").split("_");
							if(sColour.length<3)
								break;
							...
							double alpha = Integer.parseInt(data[6].replaceAll(QUOTEMARK, ""))/10000D;

							RenderElement re = new RenderElement().setModel(modelPath).setPart(partName).setTexture(texture).setTranslation(translation[0], translation[1], translation[2]).setRotation(rotation[0], rotation[1], rotation[2]).setColour(colourModifier[0], colourModifier[1], colourModifier[2]).setAlpha(alpha);
	...
	public void savePresetRender(String name, RenderElement[] elements)
	{
		...
		for(int r=0; r<elements.length; r++)
		{
			...
			data[0] = QUOTEMARK+ re.getModel() +QUOTEMARK;
			data[1] = QUOTEMARK+ re.getPart() +QUOTEMARK;
			data[2] = QUOTEMARK+ re.getTexture() +QUOTEMARK;
			data[3] = QUOTEMARK+ ( (int)Math.floor(re.getTranslation()[0]*100D) ) +"_"+ ( (int)Math.floor(re.getTranslation()[1]*100D) ) +"_"+ ( (int)Math.floor(re.getTranslation()[2]*100D) ) +QUOTEMARK;
			data[4] = QUOTEMARK+ ( (int)Math.floor(re.getRotation()[0]*100D) ) +"_"+ ( (int)Math.floor(re.getRotation()[1]*100D) ) +"_"+ ( (int)Math.floor(re.getRotation()[2]*100D) ) +QUOTEMARK;
			data[5] = QUOTEMARK+ ( (int)Math.floor(re.getColour()[0]*255D) ) +"_"+ ( (int)Math.floor(re.getColour()[1]*255D) ) +"_"+ ( (int)Math.floor(re.getColour()[2]*255D) ) +QUOTEMARK;
			data[6] = QUOTEMARK+ Integer.toString((int) Math.floor(re.getAlpha()*10000D)) +QUOTEMARK;
