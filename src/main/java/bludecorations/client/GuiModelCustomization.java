...
public class GuiModelCustomization extends GuiScreen
{
	...
	public void initGui()
	{
		...
		else if(renderElements.length>0)
		{
			...
			if(re!=null)
			{
				...
				this.textField_modelPath.setText(re.getModel());
				this.textField_texturePath = new GuiTextboxAdvanced(this.fontRenderer, x+7, y+131, 176, 12);
				...
				this.textField_texturePath.setText(re.getTexture());
				this.textField_renderPart = new GuiTextboxAdvanced(this.fontRenderer, x+7, y+158, 176, 12);
	...
	protected void mouseMovedOrUp(int par1, int par2, int par3)
	{
		...
		Field selBut;
		try {
			selBut = GuiScreen.class.getDeclaredFields()[6];
	...
	public void drawScreen(int par1, int par2, float par3)
	{
		...
		if(page > 0)
		{
			...
			this.fontRenderer.drawStringWithShadow("Model Path:", k+8, l+92, GraphicUtilities.isModelPathValid(textField_modelPath.getText()) ? 14737632 : 9125927);
			this.fontRenderer.drawStringWithShadow("Texture Path:", k+8, l+119, GraphicUtilities.isTexturePathValid(textField_texturePath.getText()) ? 14737632 : 9125927);
			boolean validModelPart = false;
			if(GraphicUtilities.isModelPathValid(textField_modelPath.getText()) && GraphicUtilities.bindModel(textField_modelPath.getText())!=null)
	...
	protected void actionPerformed(GuiButton button)
	{
		...
		if(button.id == 3)
		{
			try{
				...
				PacketHandler.sendRenderWipePacket(tileEntity.worldObj, tileEntity.xCoord, tileEntity.yCoord, tileEntity.zCoord);
				tileEntity.setRenderElements(rList.toArray(new RenderElement[rList.size()]));
				updateTile();
				page = 0;
				this.initGui();
	...
	public void updateTile()
	{
		if(page != 0)
		{
			...
			{
				if(textField_modelPath != null)
					re.setModel(this.textField_modelPath.getText());
				if(textField_texturePath != null)
					re.setTexture(this.textField_texturePath.getText());
				if(textField_renderPart != null)
