...
public class ParticleElement
{
	...
	public void writeToOutputStream(DataOutputStream dataStream) throws IOException
	{
		...
		dataStream.writeFloat(colourModifier != null ? colourModifier[0]: 0);
		dataStream.writeFloat(colourModifier != null ? colourModifier[1]: 0);
		dataStream.writeFloat(colourModifier != null ? colourModifier[2]: 0);
		dataStream.writeFloat(scale);
		dataStream.writeFloat(alpha);
	}

	public static ParticleElement readFromInputStream(DataInputStream data) throws IOException
	{
		ParticleElement element = new ParticleElement();
		element.setParticleClass(data.readUTF());
		double trX = data.readDouble();
		double trY = data.readDouble();
		double trZ = data.readDouble();
		element.setTranslation(trX,trY,trZ);
		float colX = data.readFloat();
		float colY = data.readFloat();
		float colZ = data.readFloat();
		element.setColour(colX,colY,colZ);
		element.setScale(data.readFloat());
		element.setAlpha(data.readFloat());
		return element.update();
	}
